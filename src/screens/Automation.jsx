...
function Builder() {
  ...
  if (view === 'list') {
    ...
    const metricBox = (label, value, danger) => (
      <div key={label} style={{ background: '#F6FAF6', border: '1px solid rgba(27,76,94,.08)', borderRadius: 10, padding: '10px 12px', flex: 1, minWidth: isMobile ? '40%' : 96 }}>
        <div style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', color: 'rgba(27,76,94,.5)' }}>{String(label).toUpperCase()}</div>
    ...
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '18px 16px 28px' : '22px 30px 36px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12, flexWrap: 'wrap' }}>
          <div style={{ fontSize: 14.5, fontWeight: 800, color: FOREST }}>Workflows <span style={{ color: 'rgba(27,76,94,.45)', fontWeight: 700 }}>({flowList.length})</span></div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={refreshList} style={{ background: '#fff', border: '1px solid rgba(27,76,94,.16)', color: FOREST, fontSize: 12.5, fontWeight: 700, padding: '9px 14px', borderRadius: 10, cursor: 'pointer' }}>Refresh</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {flowList.map(f => {
            ...
            return (
              <div key={f.id} style={{ background: '#fff', border: '1px solid rgba(27,76,94,.10)', borderRadius: 14, padding: '16px 18px', boxShadow: '0 1px 3px rgba(14,58,53,.05)' }}>
        ...
        <div style={{ fontSize: 11, color: 'rgba(27,76,94,.4)', marginTop: 16, lineHeight: 1.55 }}>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
      {/* Palette */}
      <div style={{ width: isMobile ? 150 : 220, flexShrink: 0, background: '#fff', borderRight: '1px solid rgba(27,76,94,.10)', overflowY: 'auto', padding: isMobile ? '12px 8px' : '16px 12px' }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: FOREST, padding: '0 4px 6px' }}>Blocks</div>
        <div style={{ fontSize: 11, color: 'rgba(27,76,94,.5)', padding: '0 4px 14px', lineHeight: 1.45 }}>Drag a block onto the canvas, then connect them.</div>
        {PALETTE.map(group => (
          <div key={group.group} style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.1em', color: 'rgba(27,76,94,.42)', marginBottom: 8, padding: '0 4px' }}>{group.group}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {group.items.map((it, i) => (
                <div key={i} draggable
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', borderRadius: 10, border: '1px solid rgba(27,76,94,.12)', background: '#fff', cursor: 'grab', userSelect: 'none' }}>
                  <span style={{ width: 28, height: 28, borderRadius: 8, background: '#F2F8F2', color: 'var(--brand-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 6, flexShrink: 0 }}>
                    <it.Icon size={16} />
                  </span>
      ...
      <div style={{ flex: 1, minWidth: 0, position: 'relative' }} onDrop={onDrop} onDragOver={onDragOver}>
        ...
        <div style={{ position: 'absolute', top: 14, left: 16, right: 16, zIndex: 5, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, rowGap: 8, pointerEvents: 'none', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, rowGap: 8, pointerEvents: 'auto', flexWrap: 'wrap' }}>
            <button onClick={() => { setView('list'); refreshList(); }} title="All workflows" style={{ background: '#fff', border: '1px solid rgba(27,76,94,.16)', borderRadius: 10, padding: '9px 13px', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: FOREST, boxShadow: '0 2px 8px rgba(14,58,53,.06)' }}>← All</button>
            {/* Flow picker */}
            <div style={{ position: 'relative' }}>
              <button onClick={() => setPickerOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1px solid rgba(27,76,94,.16)', borderRadius: 10, padding: '9px 12px', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: FOREST, boxShadow: '0 2px 8px rgba(14,58,53,.06)' }}>
                <span style={{ maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{flowName || 'Flows'}</span>
                <IconChevDown size={13} />
              </button>
              {pickerOpen && (
                <div style={{ position: 'absolute', top: '112%', left: 0, background: '#fff', border: '1px solid rgba(27,76,94,.14)', borderRadius: 11, boxShadow: '0 10px 28px rgba(14,58,53,.14)', minWidth: 260, overflow: 'hidden', zIndex: 20 }}>
                  {flowList.map(f => {
                    const fs = statusStyle(f.status);
                    return (
                      <button key={f.id} onClick={() => { loadFlow(f.id); setPickerOpen(false); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, width: '100%', textAlign: 'left', padding: '10px 13px', background: f.id === flowId ? '#F2F8F2' : '#fff', border: 'none', borderBottom: '1px solid rgba(27,76,94,.06)', cursor: 'pointer', fontSize: 13, color: FOREST, fontWeight: f.id === flowId ? 700 : 500 }}>
            ...
            <input value={flowName} onChange={e => setFlowName(e.target.value)} placeholder="Flow name"
              style={{ background: '#fff', border: '1px solid rgba(27,76,94,.16)', borderRadius: 10, padding: '9px 13px', fontSize: 14, fontWeight: 700, color: FOREST, outline: 'none', fontFamily: 'inherit', minWidth: isMobile ? 120 : 220, maxWidth: isMobile ? 160 : 'none', boxShadow: '0 2px 8px rgba(14,58,53,.06)' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, rowGap: 8, pointerEvents: 'auto', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {toast && <span style={{ background: 'var(--brand-primary-dark)', color: 'var(--brand-accent-pale)', fontSize: 12, fontWeight: 700, padding: '7px 13px', borderRadius: 999 }}>{toast}</span>}
            <span style={{ position: 'relative' }}>
              <button onClick={() => { setShowTest(v => !v); setTestResult(null); }} style={{ background: showTest ? '#EAF6E4' : '#fff', border: '1px solid rgba(27,76,94,.16)', color: FOREST, fontSize: 13, fontWeight: 700, padding: '9px 15px', borderRadius: 10, cursor: 'pointer' }}>Test</button>
              {showTest && (
                <>
                  <div onClick={() => setShowTest(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
                  <div style={{ position: 'absolute', top: '120%', right: 0, width: 300, background: '#fff', border: '1px solid rgba(27,76,94,.14)', borderRadius: 12, boxShadow: '0 12px 32px rgba(14,58,53,.18)', zIndex: 50, padding: 16 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 800, color: FOREST, marginBottom: 4 }}>Send a test</div>
                    <div style={{ fontSize: 11.5, color: 'rgba(27,76,94,.55)', marginBottom: 10, lineHeight: 1.45 }}>Sends this flow's template to the number below (must be a contact in People). Variables fill from that contact.</div>
            ...
            <button onClick={handleDelete} title="Delete flow" style={{ background: '#fff', border: '1px solid rgba(27,76,94,.16)', color: '#C7503B', fontSize: 13, fontWeight: 700, padding: '9px 13px', borderRadius: 10, cursor: 'pointer' }}>Delete</button>
            <button onClick={() => persist()} disabled={saving} style={{ background: '#fff', border: '1px solid rgba(27,76,94,.16)', color: FOREST, fontSize: 13, fontWeight: 700, padding: '9px 15px', borderRadius: 10, cursor: 'pointer' }}>{saving ? 'Saving…' : 'Save'}</button>
            {status === 'active'
        ...
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(27,76,94,.5)', fontSize: 14, zIndex: 4 }}>Loading flows…</div>
        )}
        ...
        >
          <Background variant={BackgroundVariant.Dots} gap={20} size={1.5} color="rgba(27,76,94,.18)" />
          <Controls />
          <MiniMap pannable zoomable nodeColor="var(--brand-accent-pale)" maskColor="rgba(27,76,94,.06)" style={{ background: '#fff', border: '1px solid rgba(27,76,94,.12)' }} />
        </ReactFlow>
      </div>
...
export default function Automation() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <header style={{ padding: isMobile ? '14px 16px 12px' : '18px 30px 14px', borderBottom: '1px solid rgba(27,76,94,.08)', background: '#fff' }}>
        <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.12em', color: 'rgba(27,76,94,.45)' }}>AUTOMATION</div>
